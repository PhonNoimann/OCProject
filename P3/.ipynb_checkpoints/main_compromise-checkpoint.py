...
for w1 in W1:
    for w2 in W2:
        ...
        w = [w1,w2,w3]
        model.obj = Objective(
            expr = ((obj_file['x1'][0]*model.x['x1']+obj_file['x2'][0]*model.x['x2'])**4)*w[0] + ((obj_file['x1'][1]*model.x['x1']+obj_file['x2'][1]*model.x['x2'])**4)*w[1] + ((obj_file['x1'][2]*model.x['x1']+obj_file['x2'][2]*model.x['x2'])**4)*w[2], sense = minimize)
...
plt.savefig('compromise_x.png')

"""
fig2, ax2 = plt.subplots()
ax2.plot(z1,z2,'.r')
ax2.plot(z1,z2,)
ax2.set_title('Pareto frontier Compromise method z\'s values')
ax2.set_xlabel('z1')
ax2.set_ylabel('z2')
ax2.grid(True)
